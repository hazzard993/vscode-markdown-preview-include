...
const DEFAULT_CIRCULAR_MESSAGE: string =
  "Circular reference between '{{FILE}}' and '{{PARENT}}'";

// Default regex patterns
const COMMONMARK_PATTERN: RegExp =
  /\:(?:\[([^|\]]*)\|?([^\]]*)\])?\(([^)]+)\)/i;
const MARKDOWN_IT_PATTERN: RegExp =
...
export = function Include(markdown: MarkdownIt, settings: IncludeSettings) {
  ...
  function replace(
    ...
    if (fs.existsSync(childFile) === false) {
      ...
      childContent = circulareMessage
        .replace("{{FILE}}", childFile)
        .replace("{{PARENT}}", parentFile as string);
      error = true;
    } else {
    ...
    let parentContentEnd = parentContent.slice(
  ...
  function execute(
    ...
    if (
      settings.commonmarkRegex === undefined
        ? DEFAULT_COMMONMARK_REGEX
        : settings.commonmarkRegex
    ) {
    ...
    if (
      settings.markdownItRegex === undefined
        ? DEFAULT_MARKDOWN_IT_REGEX
        : settings.markdownItRegex
    ) {
